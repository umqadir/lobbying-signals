...
- [x] Run one clean manual refresh in GitHub Actions.
  ...
  - Scheduled refresh now requires only `LDA_API_KEY`; Gemini secrets are no longer used by the default pipeline.
...
- [x] Add a short methodology/data note.
  - Minimum note: source is Senate LDA filings; mentions are deterministic rule matches on activity text, not unique filings; comparisons are directional signals, not causal claims.
